...
ReceiptStateList?:ReceiptStateList[]=[];
ReceiptStateListTab?:ReceiptStateList[]=[];
valdata="";valuid=0;
...
settingtype=''
  ...
  apply(filterValue:string) {
    ...
    this.ReceiptStateListTab=[];
  
  this.dataSource =new MatTableDataSource<any>(this.ReceiptStateListTab);
  ...
  onSubmit() {
   ...
   this.isDisable=true;
      ...
      else
      {
        ...
        this.hwServices.AddReceiptStatus(this.receipt).subscribe(res=>
          {
            if(res.status==true)
            {
